...
1. SAP에 데이터를 추가하는 작업을 만들어 보겠습니다.</br>
   [make.powerautomate.com](https://make.powerautomate.com)에 접속하고, **SAP 품목 정보 변경** 흐름을 **SAP 품목 등록**으로 **다른 이름으로 저장**합니다.
...
4. **OData 엔터티 만들기** 커넥터의 **OData 엔터티 이름** 옵션을 **ProductSet**으로 설정합니다.</br>
   8개의 필수 매개변수가 나타납니다. 필수 매개변수 중 4개는 기본값을 정해 미리 입력하겠습니다.</br>
...
5. 고급 매개 변수에서 **Description**과 **Price**를 추가하고, 각각 값에 변수를 입력합니다.
...
7. Copilot Studio로 돌아와서, 새로 토픽을 생성합니다.</br>
   토픽 이름과 토픽이 수행하는 작업을 아래와 같이 입력합니다.</br>
   **토픽 이름**
   ```
   SAP Product Create
   ```
...
8. **토픽 세부 정보**-**입력**에 변수를 설정합니다.</br>
   ...
   **설명**
   ...
   <img src="https://github.com/user-attachments/assets/bc803a79-9579-4d9f-89e6-d11ae7d0ed6d" width=500/>
...
12. 아래에 **적응형 카드로 질문**을 추가합니다. 속성을 수식 카드로 변경하고, [생성용 응형 카드 탬플릿](https://github.com/FDX-edu/CopilotStudioSAP/blob/main/Resources/AdaptiveCardCreate.json)의 내용을 복사하여 붙여넣습니다.

    <img src="https://github.com/user-attachments/assets/e439d379-77f7-4643-a3da-776ce8d5c45f" width=800/>

    스키마 편집은 아래의 값을 복사하여 붙여넣습니다.
    ```
    kind: Record
    properties:
      category: String
...
14. 이번에는 입력한 제품의 id가 이미 존재할 경우 처리를 하겠습니다.</br>
    ...
    💡스키마 값은 [2-2 아이디별 조회의 14번 이후](https://github.com/FDX-edu/CopilotStudioSAP/blob/main/2.%20SAP%20%EC%A1%B0%ED%9A%8C/2-2.%20%EC%95%84%EC%9D%B4%EB%94%94%EB%B3%84%20%EC%A1%B0%ED%9A%8C.md)를 참고하세요.


15. **메시지** 노드를 추가하고, **+ 추가** 버튼에서 **적응형 카드**를 추가합니다.</br>
    속성을 **수식 카드**로 변경하고, [조회용 적응형 카드 탬플릿](https://github.com/FDX-edu/CopilotStudioSAP/blob/main/Resources/AdaptiveCardRead.json)의 내용을 복사하여 붙여넣습니다.

    <img src="https://github.com/user-attachments/assets/01d291ff-9cf6-4d93-b078-98501cda9902" width=800/>

    마지막으로 다시 질문으로 되돌아갈 수 있도록 **단계로 이동**을 추가하고, 토픽을 저장합니다.
...
16. 테스트를 통해 결과를 확인해 보겠습니다.
    ...
    1) 이미 존재하는 제품을 등록하려고 하면, 제품이 이미 존재함을 알리며 그 제품의 정보를 출력합니다.</br>
    2) 존재하지 않는 제품명을 전달하면 그 제품명으로 등록을 시도합니다. 임의의 값을 채워넣어 진행합니다.</br>
    3) 등록이 완료된 이후 상세조회해 보면, 등록할 때 작성했던 값들로 제대로 채워진 것을 확인할 수 있습니다.
